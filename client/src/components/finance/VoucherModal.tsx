...
export const VoucherModal: React.FC<VoucherModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 print:p-0 print:bg-white print:static print:overflow-visible">
      {/* Container */}
      <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-[#DDE5F0] overflow-hidden flex flex-col max-h-[92vh] print:max-h-none print:shadow-none print:border-none print:rounded-none">
        {/* Modal Action Bar (Hidden in print) */}
        <div className="flex items-center justify-between px-6 py-4 bg-[#F6F8FC] border-b border-[#DDE5F0] print:hidden">
          <div className="flex items-center gap-2 text-[#475569]">
            <Building2 className="w-5 h-5 text-[#12325C]" />
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#12325C] hover:bg-[#0F2947] text-white font-medium text-xs shadow-xs transition-colors"
            >
            ...
            <button
              onClick={onClose}
              className="p-2 text-[#64748B] hover:text-[#0C1B33] hover:bg-black/5 rounded-xl transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="p-6 md:p-10 overflow-y-auto print:p-0 print:overflow-visible">
          ...
          <div
            ref={printAreaRef}
            className="voucher-print-area max-w-2xl mx-auto bg-white border border-slate-300 print:border-none p-8 sm:p-10 rounded-2xl text-slate-900 font-sans"
          >
            {/* Header / Church Info */}
            <div className="border-b-2 border-slate-800 pb-5 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
            ...
            <div className="text-center my-6">
              <span className="inline-block px-6 py-1.5 bg-slate-100 border border-slate-300 rounded-lg text-sm sm:text-base font-bold text-slate-900 tracking-wide">
                {docTitle}
              </span>
            </div>

            {/* Info Grid */}
            <div className="grid grid-cols-2 gap-4 text-xs mb-6 bg-slate-50/70 p-4 rounded-xl border border-slate-200">
              <div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tfoot className="bg-slate-50 border-t-2 border-slate-300 font-bold">
                  <tr>
                    ...
                    <td className="py-3 px-4 text-right text-base text-slate-900 font-mono">
                      {data.amount.toLocaleString("th-TH", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
