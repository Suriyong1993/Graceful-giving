...
const TONE_STYLES = {
  ok: { bar: "bg-[#047857]", text: "text-[#065F46]", label: "อยู่ในงบ" },
  warn: { bar: "bg-[#12325C]", text: "text-[#0F2947]", label: "ใกล้เต็มงบ" },
  over: { bar: "bg-[#B91C1C]", text: "text-[#B91C1C]", label: "เกินงบ" },
} as const;
...
export const BudgetProgress: React.FC<{
  ...
  return (
    <div className={className}>
      <div
        className="h-2.5 w-full overflow-hidden rounded-full bg-[#DCE4F0]"
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={100}
      ...
      <div className="mt-1.5 flex items-center justify-between text-xs tabular-nums">
        ...
        <span className="text-[#64748B]">ใช้ไป {percent.toFixed(0)}%</span>
      </div>
    </div>
  );
};
...
const inputClass =
  "mt-1 min-h-11 w-full rounded-xl border border-[#DDE5F0] bg-white p-3 text-base font-normal text-[#0C1B33] md:text-sm";
