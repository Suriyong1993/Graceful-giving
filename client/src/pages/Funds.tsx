    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {fundsList.length === 0 && (
            <p className="md:col-span-2 lg:col-span-3 py-12 text-center text-sm text-[#64748B] bg-white rounded-2xl border border-dashed border-[#DDE5F0]">
          ...
          {fundsList.map(f => {
            ...
            return (
              <div
                key={f.id}
                className="bg-white rounded-2xl border border-[#DDE5F0] p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group cursor-pointer"
                onClick={() => setLocation(`/funds/${f.id}`)}
              >
                <div className="space-y-4">
                  {/* Top Bar */}
                  <div className="flex items-center justify-between">
                    <div className="w-12 h-12 rounded-2xl bg-[#EDF1F7] flex items-center justify-center text-[#475569] group-hover:scale-105 transition-transform">
                      <Icon className="w-6 h-6 text-primary" />
                    </div>
                    <span className="text-xs font-mono text-[#64748B] bg-background px-2.5 py-1 rounded-full border border-[#DDE5F0]">
                      {f.code}
                    </span>
                  </div>
                  ...
                  {/* Balance Display */}
                  <div className="pt-2">
                    <p className="text-xs text-[#64748B]">ยอดคงเหลือสุทธิ</p>
                    <div
                      className={`text-2xl font-bold tabular-nums ${f.balance < 0 ? "text-[#B91C1C]" : "text-[#0C1B33]"}`}
                    >
                  ...
                  <div className="pt-2 border-t border-[#DDE5F0]/40 text-xs text-[#64748B]">
                    กิจกรรมล่าสุดจะแสดงเมื่อมีข้อมูลจากระบบ
                  </div>
                </div>

                {/* Bottom Action */}
                <div className="pt-5 mt-4 border-t border-[#DDE5F0]/50 flex items-center justify-between text-xs font-semibold text-[#475569] group-hover:text-primary">
        ...
        {showNewFundModal && (
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl border border-[#DDE5F0] max-w-md w-full max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain p-6 md:p-8 space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between border-b border-[#DDE5F0] pb-3">
                ...
                <button
                  ...
                  type="button"
                  aria-label="ปิด"
                  className="-mr-2 flex size-11 shrink-0 items-center justify-center rounded-xl text-xl font-bold text-[#64748B] hover:bg-[#EDF1F7] hover:text-foreground"
                >
                  ×
                </button>
              </div>
              ...
              <form onSubmit={handleCreateFund} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-2xl border border-[#DDE5F0] text-sm focus:border-primary focus:outline-none"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-3">
                  <button
                    type="button"
                    onClick={() => setShowNewFundModal(false)}
                    className="px-4 py-2.5 rounded-xl border border-[#DDE5F0] text-xs font-medium text-[#475569]"
                  >
                    ยกเลิก
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2.5 rounded-xl bg-primary text-white text-xs font-semibold hover:bg-[#0F2947]"
                  >
                    สร้างกองทุน
                  </button>
                </div>
