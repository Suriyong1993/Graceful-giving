...
import { CalendarClock, Plus, UserRound, X } from "lucide-react";
      ...
      action={
        canManage ? (
          <button
            ...
            className="min-h-11 inline-flex items-center gap-2 rounded-2xl bg-brand px-4 py-2 text-xs font-bold text-white"
          >
      ...
      <div className="space-y-6">
        <section className="rounded-3xl border border-line bg-sunken p-6 shadow-sm md:p-8">
          <div className="flex items-start gap-4">
            <div>
              <h1 className="text-xl font-bold text-ink">พันธกิจและฝ่ายงาน</h1>
        ...
        {canManage && showCreate && (
          ...
          >
            <div className="mb-5 flex items-center justify-between">
              <h2 className="font-bold text-ink">เพิ่มฝ่ายงานใหม่</h2>
              <button
                type="button"
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="text-sm font-semibold text-ink-2">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-xl border border-line p-3 font-normal text-ink"
                />
              </label>
              <label className="text-sm font-semibold text-ink-2">
                หัวหน้าฝ่าย
            ...
            <button
              ...
              className="mt-5 min-h-11 rounded-2xl bg-success px-5 py-2 text-sm font-bold text-white disabled:opacity-50"
            >
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {ministriesQuery.data.map(ministry => (
              <button
                ...
                className="rounded-2xl border border-line bg-card p-5 text-left shadow-sm hover:bg-page"
              >
                <div className="flex items-center justify-between gap-3">
                  <h2 className="font-bold text-ink">{ministry.name}</h2>
                  <span
                ...
                <p className="mt-2 flex items-center gap-1.5 text-sm text-ink-2/80">
                  <UserRound className="h-4 w-4 shrink-0" />
                  {ministry.leaderName || "ยังไม่ระบุหัวหน้าฝ่าย"}
                </p>
                <p className="mt-1 flex items-center gap-1.5 text-sm text-ink-2/80">
