  ...
  return (
        ...
        <div className="flex items-center gap-2">
          {transaction && (
            <button
              ...
              className="min-h-11 px-3.5 py-2 rounded-xl bg-[#EEF1F3] hover:bg-[#225B66] hover:text-white text-[#42515A] text-xs font-bold border border-[#DCE3E6] flex items-center gap-1.5 transition-colors shadow-2xs"
            >
    ...
    >
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-[#DCE3E6] card-elevation-sm space-y-6">
          {isEditing && (
            <form
              onSubmit={submitEdit}
              className="rounded-2xl bg-[#FAF8F5] border border-[#DCE3E6] p-4 space-y-3"
            >
              <p className="text-sm font-bold text-[#172128]">
                แก้ไขข้อมูลรายการ
              </p>
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="text-xs font-semibold text-[#42515A]">
                  จำนวนเงิน
                  <input
                    type="number"
                    min="0.01"
                    ...
                    className="mt-1 w-full rounded-xl border border-[#DCE3E6] p-3 text-sm"
                  />
                </label>
                <label className="text-xs font-semibold text-[#42515A]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#DCE3E6]/60">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-[#6A7880]">
                  {transaction.type === "income" ? "รายรับ (ถวาย)" : "รายจ่าย"}
                </span>
                <StatusBadge status={transaction.status} />
              </div>
              <h2 className="text-xl sm:text-2xl font-bold text-[#172128]">
                {transaction.title}
              </h2>
              <p className="text-xs text-[#6A7880] flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5 text-[#225B66]" />
                {new Date(transaction.date).toLocaleString("th-TH")}
              </p>
            </div>
            <div className="text-left sm:text-right">
              <span className="text-xs text-[#6A7880] block">
                จำนวนเงินสุทธิ
              </span>
              <MoneyDisplay
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <Detail
              ...
              icon={<Landmark className="w-4 h-4 text-[#225B66]" />}
            />
          ...
          {transaction.notes && (
            <div className="rounded-2xl bg-[#FFFFFF] border border-[#DCE3E6]/70 p-4">
              <p className="text-xs text-[#6A7880]">หมายเหตุ</p>
              <p className="text-sm text-[#172128] mt-1">{transaction.notes}</p>
            </div>
          )}

          {/* Receipt Attachment from Supabase Storage */}
          {transaction.receiptUrl && (
            <div className="rounded-2xl bg-[#FFFFFF] border border-[#DCE3E6]/70 p-5 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#42515A] flex items-center gap-1.5">
                  <Paperclip className="w-4 h-4 text-[#225B66]" />
              ...
              >
                {transaction.receiptUrl.toLowerCase().includes(".pdf") ? (
                  <div className="text-center p-4">
                    <FileText className="w-12 h-12 text-[#225B66] mx-auto mb-2" />
...
function Detail({
  ...
  return (
    <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-[#DCE3E6]/70 space-y-1">
      <span className="text-xs text-[#6A7880] block">{label}</span>
      <span className="text-sm font-bold text-[#42515A] flex items-center gap-1.5">
        {icon}
        {value}
      </span>
    </div>
