  ...
  return (
    <section className="space-y-4">
      <div className="overflow-hidden rounded-3xl border border-[#DDE5F0] bg-white shadow-sm">
        <div className="border-b border-[#DDE5F0] p-4">
          <h2 className="font-bold text-[#0C1B33]">ใบนับธนบัตรและเหรียญ</h2>
          ...
          {THB_DENOMINATIONS.map(denomination => {
            ...
            return (
              <li key={key} className="flex items-center gap-3 p-3 md:p-4">
                <span className="w-28 shrink-0 text-sm font-bold text-[#0C1B33] md:w-40">
                  {denomination.label}
                </span>
                <input
                  type="number"
                  ...
                  className="w-24 rounded-xl border border-[#DDE5F0] p-2.5 text-right text-base font-bold tabular-nums text-[#0C1B33] disabled:opacity-60"
                />
                <span className="ml-auto text-right text-sm font-bold tabular-nums text-[#475569]">
                  {fmtBaht(subtotal)}
                </span>
              </li>
            );
        ...
        <div className="flex items-center justify-between border-t-2 border-[#DDE5F0] bg-[#F6F8FC] p-4">
          <div>
            <p className="font-bold text-[#0C1B33]">รวมนับได้</p>
