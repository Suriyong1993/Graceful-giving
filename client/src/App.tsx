  ...
  return (
    ...
    >
      <div className="w-full max-w-md space-y-3">
        <div className="h-8 w-40 rounded-xl bg-[#DDE5F0]/60 animate-pulse" />
        <div className="h-24 w-full rounded-2xl bg-[#DDE5F0]/40 animate-pulse" />
        <p className="text-center text-sm text-[#64748B]">กำลังโหลดหน้า…</p>
      </div>
    </div>
  );
}
  ...
  if (!user || !canAccess(user)) {
    return (
      <div className="min-h-screen bg-[#F6F8FC] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 border-2 border-[#DDE5F0] text-center space-y-4 shadow-sm">
          ...
          <h2 className="text-xl font-bold text-[#0C1B33]">{title}</h2>
          <p className="text-sm text-[#64748B] leading-relaxed">{message}</p>
          <div className="pt-2">
            <a
              href="/"
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-2xl bg-[#12325C] text-white font-bold text-sm hover:bg-[#0F2947] transition-all shadow-xs"
            >
              กลับสู่หน้าหลัก
            </a>
          </div>
